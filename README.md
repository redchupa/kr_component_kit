...
- 기상청 **기상특보** 데이터셋 활용신청 필요 (`data.go.kr`, 운영기관 코드 `1360000`, endpoint `WthrWrnInfoService`)
...
- 위/경도 기준으로 거리 자동 계산
- 기상청 **지진정보** 데이터셋 활용신청 필요 (`data.go.kr`, 운영기관 코드 `1360000`, endpoint `EqkInfoService`)
...
- 기상청 **단기예보** 데이터셋 활용신청 필요 (`data.go.kr`, 운영기관 코드 `1360000`, endpoint `VilageFcstInfoService_2.0`)
...
- 측정소 단위 다중 등록
- 한국환경공단 **대기오염정보** + **측정소정보** 두 데이터셋 활용신청 필요 (`data.go.kr`, 운영기관 코드 `B552584`, endpoint `ArpltnInforInqireSvc` + `MsrstnInfoInqireSvc`)
- (옵션) UV / 대기정체 sensor 추가 시 기상청 **생활기상지수** 활용신청 필요 — ⚠️ V4/V5 호환성 주의, 자세한 안내는 [에어코리아 API 가이드 섹션](#-에어코리아--필수-2건--옵션-1건) 참고
...
- **국립중앙의료원_전국 약국 정보 조회 서비스** 키 필요 (`data.go.kr`, 운영기관 코드 B552657)
...
> ℹ️ 표 안의 정보는 두 가지로 구분됩니다:
> - **✅ 검증됨** — 본 컴포넌트가 호출하는 endpoint URL과 운영기관 코드는 [소스 코드](custom_components/kr_component_kit/)에서 직접 확인한 사실
> - **🔎 검색 키워드 / 등록명** — data.go.kr 포털 측에서 데이터셋 이름은 가끔 바뀌므로, 등록명 일치가 아닌 **검색 키워드 + 운영기관 + endpoint URL** 3중 기준으로 찾으세요
>
> 활용신청은 *데이터셋 단위* 로 따로 해야 합니다. 같은 인증키라도 신청 안 한 데이터셋은 호출 시 `403` 을 받습니다.

| 사용할 서비스 | 발급 포털 | 검색 키워드 | 운영기관 / Endpoint URL (검증됨) |
|---|---|---|---|
| 💊 **약국** | [공공데이터포털](https://www.data.go.kr) | `전국 약국` · `약국 정보` | 국립중앙의료원 (`B552657`) · `B552657/ErmctInsttInfoInqireService` |
| 📢 **재난문자** | [행정안전부 안전데이터포털](https://www.safetydata.go.kr) | `재난문자` | 행정안전부 · `safetydata.go.kr/V2/api/DSSP-IF-00247` |
| 🌪️ **기상특보** | [공공데이터포털](https://www.data.go.kr) | `기상특보` | 기상청 (`1360000`) · `1360000/WthrWrnInfoService` |
| 🌍 **지진** | [공공데이터포털](https://www.data.go.kr) | `지진정보` | 기상청 (`1360000`) · `1360000/EqkInfoService` |
| ⛅ **동네예보** | [공공데이터포털](https://www.data.go.kr) | `단기예보` · `동네예보` | 기상청 (`1360000`) · `1360000/VilageFcstInfoService_2.0` |
| 🌫️ **에어코리아 (2건 필수)** | [공공데이터포털](https://www.data.go.kr) | `대기오염정보` + `측정소정보` | 한국환경공단 (`B552584`) · `B552584/ArpltnInforInqireSvc` + `B552584/MsrstnInfoInqireSvc` *(두 데이터셋 각각 활용신청 필요)* |
| 🌫️ **에어코리아 (옵션 — UV/대기정체)** | [공공데이터포털](https://www.data.go.kr) | `생활기상지수` | 기상청 (`1360000`) · `1360000/LivingWthrIdxServiceV4` ⚠️ *(V5로 업그레이드 진행 중일 수 있음 — [관련 이슈](#-에어코리아-생활기상지수-v4v5-주의) 참고)* |
| ⛽ **유가** | [Opinet 오피넷](https://www.opinet.co.kr/user/api/empApiInfo.do) | 무료 회원가입 → API 신청 (공공데이터포털과 별개) | Opinet · `www.opinet.co.kr/api` |
| 🏫 **학교** | [NEIS 교육정보 개방 포털](https://open.neis.go.kr) | 회원가입 → 인증키 발급 (공공데이터포털과 별개) | NEIS · `open.neis.go.kr/hub` |
| 🚌 **대중교통(지하철)** | [서울 열린데이터광장](https://data.seoul.go.kr) | `지하철 실시간 도착정보` | 서울특별시 · `swopenAPI.seoul.go.kr` |
| 🚌 **대중교통(버스)** | [공공데이터포털](https://www.data.go.kr) | `버스도착정보` 또는 `TAGO` | 국토교통부 · `apis.data.go.kr/...버스도착정보` 계열 |
...
약국 데이터는 **국립중앙의료원(NMC)** 이 제공하며 **공공데이터포털(data.go.kr)** 에서 받습니다.

**단계별 안내:**

1. **공공데이터포털 가입** — <https://www.data.go.kr> → 우측 상단 회원가입 (이메일/카카오/네이버 OAuth 가능)
2. **데이터셋 검색** — 상단 검색창에 `전국 약국` 또는 `약국 정보` 입력
3. **결과 중 선택** — 운영기관 **국립중앙의료원**, 등록명 **"국립중앙의료원_전국 약국 정보 조회 서비스"** 항목 선택 *(운영기관 코드 `B552657`)*
...
> 💡 **코드 호출 endpoint** *(참고용)*: `apis.data.go.kr/B552657/ErmctInsttInfoInqireService/getParmacyListInfoInqire`. 데이터셋 등록명은 포털 정책에 따라 변경될 수 있으니, 일치하지 않으면 **운영기관 코드 `B552657`** 로도 검색 가능합니다.
...
2. 상단 검색에서 `재난문자` 검색 → 본 컴포넌트가 호출하는 endpoint는 **`safetydata.go.kr/V2/api/DSSP-IF-00247`** 입니다. 결과 항목 중 식별번호 `DSSP-IF-00247` 또는 동일 URL이 명시된 데이터셋을 선택하세요.
3. **활용신청 → 인증키 발급**
...
### 🌪️🌍⛅ 기상청 (특보/지진/단기예보) — 한 키로 OK

기상청 API 3종은 **모두 공공데이터포털에서 받지만 데이터셋이 각각 다르므로 활용신청을 3번 해야 합니다.** 인증키 자체는 *내 계정 하나에 발급된 같은 키*를 그대로 사용합니다.

| 코드 호출 endpoint (검증됨) | 권장 검색 키워드 |
|---|---|
| `1360000/WthrWrnInfoService` (기상특보) | `기상특보` |
| `1360000/EqkInfoService` (지진정보) | `지진정보` |
| `1360000/VilageFcstInfoService_2.0` (단기예보) | `단기예보` |

- 각각의 데이터셋 페이지에서 **활용신청** → 마이페이지에서 *승인* 확인
- Home Assistant 각 항목에 같은 인증키를 입력
- 운영기관 코드 `1360000` (기상청) 으로도 운영기관 검색이 가능합니다.
...
**필수 (둘 다 활용신청해야 동작):**
- 검색 `대기오염정보` → **한국환경공단_에어코리아_대기오염정보** *(미세먼지·O₃·NO₂ 등 실시간 수치 + 예보)*
- 검색 `측정소정보` → **한국환경공단_에어코리아_측정소정보** *(시도별 측정소 목록)*
- 두 데이터셋은 운영기관(B552584)이 같지만 활용신청은 **각각** 해야 합니다. 측정소정보 신청이 빠지면 config flow에서 측정소 선택 dropdown이 비어 보일 수 있습니다.

**옵션 (자외선지수·대기정체지수 sensor를 쓰고 싶을 때만):**
- 검색 `생활기상지수` → 운영기관 **기상청** 데이터셋 활용신청
- ⚠️ **V4/V5 주의** — 본 컴포넌트 코드는 endpoint `LivingWthrIdxServiceV4` 를 호출하지만, 2026년 5월 기준 포털에 보이는 데이터셋은 표시명 "(3.0)" 인데 실제 endpoint는 V5 (`LivingWthrIdxServiceV5/getUVIdxV5`)로 업그레이드된 상태로 확인됩니다. 즉 코드가 V4를 부르면 정상 응답을 받지 못할 가능성이 있으니, 실제 작동 여부는 HA 로그(`설정 → 시스템 → 로그` 에서 `LivingWthrIdx` 검색)로 확인하세요. 이 기능을 안 쓰면 신청 자체가 불필요합니다.<a id="-에어코리아-생활기상지수-v4v5-주의"></a>

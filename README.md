...
| 항목 | 값 |
|---|---|
| 🌐 포털 | [행정안전부 안전데이터포털 (safetydata.go.kr)](https://www.safetydata.go.kr) |
| 🔎 바로 검색 | 위 포털 상단 검색창에 `재난문자` 입력 |
| 검색어 | `재난문자` |
| 운영기관 | 행정안전부 |
| 코드 호출 endpoint | `safetydata.go.kr/V2/api/DSSP-IF-00247` |

> ⚠️ 안전데이터포털은 공공데이터포털과 **다른 사이트**입니다. 이미 `data.go.kr` 계정이 있어도 별도 가입이 필요합니다.

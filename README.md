...
#### 5️⃣ 키 복사 → HA 입력
마이페이지 → 발급된 키의 **서비스키 값** 복사 → HA 통합 추가 시 **재난문자** 선택 → 인증키 필드에 붙여넣기.

> 💡 통합 코드가 내부적으로 URL 인코딩을 수행합니다. 발급 화면에 키가 두 형태(Decoding / Encoding) 로 나오면 **Decoding(원본 문자열)** 을 사용하세요. Encoding(`%XX` 가 들어간 형태) 을 넣으면 이중 인코딩으로 서버가 거부합니다.

#### 🆘 작동 안 될 때

| 증상 | 원인 후보 | 해결 |
|---|---|---|
| `401 / SERVICE KEY ERROR` | 키 활성화 전 / 인코딩 형태 오선택 | 발급 직후라면 30분~1시간 대기 후 재시도. 안 되면 Encoding ↔ Decoding 다른 형태로 교체 |
| `403 / ACCESS_DENIED` | **IP 불일치 (가장 흔함)** | `curl -s https://api.ipify.org` 결과와 마이페이지 등록 IP 비교. 불일치면 등록 IP 수정 신청. 자주 바뀐다면 `*.*.*.*` 로 갱신 신청 |
...
> ✅ **검증 완료**: 약국 (시흥시 — 실제 sensor state=20, 20개 약국 attribute 정상). 안전알림 (시흥시 은행동 cascading 등록 성공).

...
data.go.kr 약국 정보 API의 Q1 (시군구) 파라미터에 그대로 사용 가능한
이름들로 구성됨. 행정구역이 변경되면 여기를 갱신.

마지막 갱신: 2026-05 (군위군 대구 편입, 강원/전북/전남 특별자치도 반영)
"""
from __future__ import annotations

PHARMACY_REGIONS: dict[str, list[str]] = {
    "서울특별시": [
...
SIDO_LIST: list[str] = list(PHARMACY_REGIONS.keys())


def get_sgg_list(sido: str) -> list[str]:
    """선택된 시도의 시군구 목록 반환. 시도가 없으면 빈 리스트."""
    return PHARMACY_REGIONS.get(sido, [])
